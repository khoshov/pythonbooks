...
def test_publisher_str(faker):
    publisher = baker.make(Publisher, name=faker.company(), website=faker.url())
...
def test_tag_str(faker):
    tag = baker.make(
        Tag, name=faker.word(), slug=faker.slug(), color=faker.color_name()
    )
    print(f"Tag: {tag.name} (slug: {tag.slug}, color: {tag.color})")
...
def test_book_create_and_str(faker):
    ...
    print("Tag:", tag.name, tag.slug, tag.color)
...
def test_comment_create_and_str(user, faker):
    ...
    print("User id:", comment.user.id)
    print("Book title:", comment.book.title)
